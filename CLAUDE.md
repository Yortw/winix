...
- Test classes that touch process-global state (`Environment.CurrentDirectory`, static caches/overrides, `Console.SetOut`/`SetError`) MUST share ONE xUnit collection with every other class touching the same global — per-resource collections serialise internally but still run parallel to EACH OTHER (root cause of the peep GitIgnoreChecker CI flake, commit d8a954c: a parallel CWD flip made `git check-ignore` exit 128 mid-test; same class as the protect BackendFactoryTests override race). For a project whose tests swap Console streams throughout, disable assembly parallelism instead (`[assembly: CollectionBehavior(DisableTestParallelization = true)]` — contract-tests precedent).
...
- **Doc↔behaviour reconciliation before "done" (verification oracle).** Unit tests, manual runs, and `run-smokes.sh` all confirm a command *runs and produces output* — none confirm the output *matches what the docs claim*, and all three are typically written by the same author holding the same blind spot, so they confirm the model rather than challenge it. Before claiming a tool change done, enumerate every user-facing claim across `--help`/`--describe`, `README.md`, the man page, `docs/ai/{tool}.md`, and `llms.txt`, run the command that should demonstrate each, and actively hunt for the claim that is *false*. This class has bitten repeatedly despite full test+smoke coverage: a LAN QR documented on six surfaces but never rendered; a `--color` flag that was a no-op. Platform breadth is not depth — running on Windows + WSL + macOS re-executes the same unchecked observation; one platform reconciled against its docs beats three eyeballed.
- Man pages: ~half the tools have pandoc sources (`src/{tool}/{tool}.1.md`) from which `man/man1/{tool}.1` is generated (`pandoc -s -t man {tool}.1.md -o man/man1/{tool}.1`). Before editing ANY rendered `.1`, run `git ls-files '*.1.md'` — if a source exists, edit the `.md` and regenerate, then safety-diff the `.1` (only your change + reflow may differ). Per-tool "no source exists" checks have been wrong repeatedly (4 strikes); only the repo-wide listing counts as verification.
...
- When adding a new tool:
  ...
  - Register the tool (and any subcommand `--describe` surfaces) in `tests/Winix.Contract.Tests/DescribeSurfaces.cs` and commit its snapshot under `tests/Winix.Contract.Tests/snapshots/`. Set `.Maturity(ToolMaturity.Fresh)` on the new tool's parser — it promotes to `Core` after a stable release cycle (see `docs/STABILITY.md`).
  ...
  - Never pipe a framework exception's `.Message` to user output. Use `Yort.ShellKit.SafeError.Describe(ex)` (type-maps common CoreLib exceptions to English, falls back to `ex.GetType().Name`). Adding `ex.GetType().Name` alongside the message is the acceptable minimum for broad catches. Exception: leave bespoke text that is genuinely better (a project exception with a literal English message, native-OS text from `Win32Exception`/`SocketException`). When a backend throws a CoreLib-typed exception whose `.Message` is project-authored diagnostic English (errno/HRESULT detail, secret-tool/Keychain stderr), do NOT let it fall into a broad catch's `SafeError.Describe` — that flattens the diagnostic to a bare type name. Define a typed project exception instead (precedent: `SecretStoreException`, `TrashException`, `MkAuthException`, less's `DirectoryInputException`), catch it before the broad catch, and print `.Message` verbatim.
...
src/Winix.MkAuth/          — class library (BasicAuthBuilder, BearerAuthBuilder, OAuth1Signer, JwtSigner, AzureStorageSigner, SecretRef/Resolver, Formatting, ArgParser, Cli)
src/mkauth/                — console app entry point (PackageId Winix.MkAuth)
...
src/Winix.ProcessSupervision/ — shared library (child spawn, process-tree kill-on-cancel, launch classifier, family exit codes; runfor is the first built consumer; lock/soak/attempt planned)
src/Winix.RunFor/          — class library (runfor deadline logic, formatting, Cli.Run seam)
src/runfor/                — console app entry point
...
tests/Winix.MkAuth.Tests/  — xUnit tests
...
tests/Winix.RunFor.Tests/  — xUnit tests
