...
- NuGet package IDs: `Winix.TimeIt`, `Winix.Squeeze`, `Winix.Peep`, `Winix.Wargs`. Publishing requires `NUGET_API_KEY` secret in GitHub repo settings.
- Scoop bucket: `bucket/` directory contains scoop manifests (`timeit.json`, `squeeze.json`, `peep.json`, `wargs.json`, `winix.json`). Updated automatically by the release pipeline.
