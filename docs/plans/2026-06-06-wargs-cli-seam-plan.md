...
- **Probed contract (2026-06-06, Linux binary) — pinned:** SIGINT mid-job under `--json` → stderr envelope exactly `{"tool":"wargs","version":"…","exit_code":130,"exit_reason":"cancelled"}`, wargs exits ~40ms after the signal, nothing lingers (the fixture's exit file records GNU timeout's own 124).
...
public class CliRunAsyncTests
{
    ...
    {
        // Spawn failure is the deterministic child-failure: no shell-quirk risk
        // (wargs collapses spawn failures into 123 + per-job fault_message by design).
        var r = await RunCliAsync("x\n", "--", NoSuchCommand);
    ...
    {
        // 3 items, all spawn-failures, sequential: first fails, fail-fast skips the rest.
        // Exit must be 124 (fail_fast_abort) per the round-12 SkipReason-filtered classifier.
        var r = await RunCliAsync("a\nb\nc\n", "--json", "--fail-fast", "--", NoSuchCommand);
    ...
    [Fact]
    public async Task Ndjson_EveryStderrLineParses_PerJobFields()
    {
        var r = await RunCliAsync("a\nb\n", "--ndjson", "--", NoSuchCommand);
        Assert.Equal(WargsExitCode.ChildFailed, r.Exit);
        string[] lines = r.Stderr.Split('\n', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
    ...
    public async Task Ndjson_KeepOrder_LinesInInputOrder()
    {
        ...
        var r = await RunCliAsync("one\ntwo\nthree\nfour\n",
            "--ndjson", "--keep-order", "--parallel", "4", "--", NoSuchCommand);
        string[] lines = r.Stderr.Split('\n', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
...
public class CliRunAsyncUnlockedTests
{
    ...
    /// <summary>Command argv whose appended item lands in $0 / is comment-swallowed, so the
    /// child sleeps ~30s regardless of the item (long child is load-bearing — phase-1 lesson).
    /// VERIFY at implementation: confirm the Windows `&amp;rem` form swallows the appended item
    /// (run once manually); if it does not, substitute a helper .cmd file in the test temp dir.</summary>
    private static string[] SleepCommand() =>
        OperatingSystem.IsWindows()
            ? new[] { "--", "cmd.exe", "/c", "ping -n 30 127.0.0.1 > NUL &rem" }
    ...
    [Fact]
    public async Task MidRunCancel_KillsChildren_130_Promptly()
    {
        using var stdin = new StringReader("x\n");
        var stdout = new StringWriter();
        var stderr = new StringWriter();
        using var cts = new CancellationTokenSource();
        cts.CancelAfter(300);
        var sw = System.Diagnostics.Stopwatch.StartNew();
        int exit = await Cli.RunAsync(Concat(new[] { "--json" }, SleepCommand()), stdin, stdout, stderr, cts.Token);
        sw.Stop();
        Assert.Equal(130, exit);
        string[] lines = stderr.ToString().Split('\n', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        using var doc = JsonDocument.Parse(lines[^1]);
        Assert.Equal("cancelled", doc.RootElement.GetProperty("exit_reason").GetString());
        // Coarse LIVENESS bound (not perf): must sit well under the child's ~30s sleep.
        Assert.True(sw.Elapsed < TimeSpan.FromSeconds(15),
            $"cancel→kill took {sw.Elapsed} — children were not killed promptly");
    }
...
- [ ] **Step 2: Run + full suite** — all pass; cancellation/classification contracts are pinned (probe + code-read) — failures are real signals, STOP rather than soften. The `SleepCommand` Windows `&rem` form is marked VERIFY: confirm once manually; fall back to a generated `.cmd` helper file if `&rem` doesn't swallow the appended item.
