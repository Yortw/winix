...
- **Tasks 8–10 smoke (CRITICAL catch):** real-cmd quoted-suppression smoke failed — `Environment.CommandLine` on .NET Core is NOT the raw command line (argv re-joined, quotes destroyed), so quoting detection silently saw everything unquoted. Fixed by P/Invoking `GetCommandLineW` (`NativeCommandLine.cs`); `Tokenizer_Matches_CurrentProcess` deleted (premised on the falsified assumption), `NativeCommandLine_Get_ReturnsTokenizableLine` sanity SkippableFact added; all real-cmd smokes re-run discriminating and green. This is exactly the failure class the plan's 'irreducible manual smoke layer' ship-gate existed to catch.
