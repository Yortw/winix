...
**Date:** 2026-06-02 (holistic-review follow-up: 2026-06-07)
**Status:** Accepted
...
**2026-06-07 holistic-review follow-up:** A review of the 2026-06-02 sweep found four degradations the type-name fallback introduced or left in place — CR-1 (envvault store-write degraded the backend message), CR-2 (trash flattened the SHEmptyRecycleBin HRESULT), CR-3 (protect's `DirectoryNotFoundException` arm piped the raw `IO_PathNotFound_Path` SR key), SFH-1 (digest BLAKE2b oversized-key leaked an `Arg_ParamName_Name, key` tail), and CR-4 (tests shape-shifted to assert the degraded type names). Fixed by introducing typed project exceptions `SecretStoreException` (shared `Winix.SecretStore`) and `TrashException` (`Winix.Trash`) — caught before the broad catch and printed verbatim — and by the C1/C2 point fixes. The deferred-row updates below record the dispositions.
...
| **envvault secret-store-creation broad catches** (`Program.cs:40,58`, `Cli.cs:88` via `UnwrapTypeInit`) | **RESOLVED 2026-06-07** via the typed `SecretStoreException` (shared `Winix.SecretStore`), which supersedes the wished-for "SafeError variant that prefers a non-framework inner `.Message`". The backends (libsecret/Keychain) now throw `SecretStoreException` (project-authored English, contract: safe to print verbatim) instead of `InvalidOperationException`; envvault catches it before the broad catch and prints `.Message` verbatim. A shared `Cli.DescribeSurface` helper centralises the SAFE classes — `SecretStoreException`, `Win32Exception` (native DPAPI/Cred-Mgr text), `PlatformNotSupportedException` (factory English), and the unwrapped libsecret-load `FileNotFoundException` (`FileName == null`) — printed verbatim; all other framework exceptions route through `SafeError`. The 7 guarding tests' fake now raises `SecretStoreException` (the real backend type) so the verbatim contract is exercised faithfully. **Sibling found by this holistic review:** the store-*write* path (`Cli.cs` `--value` catch, **CR-1**) was using bare `SafeError.Describe`, degrading the backend message to a type name — fixed in the same pass (now `DescribeSurface`). |
...
| **Backend throw-type contract not pinnable by consumer fakes** (TA Watch, 2026-06-07) | The Linux/macOS SecretStore backends now throw `SecretStoreException`, but no test asserts the THROWN TYPE at the backend boundary — the platform-gated backend integration tests don't cover the failure-throw paths (they need a real locked collection / missing secret-tool), and the consumer fakes hardcode `SecretStoreException` on both sides (the protocol-fake blind spot: a backend regressing to `InvalidOperationException` would degrade messages to a type name in production while every consumer test stays green). Ship gate for this specific contract is integration testing on real Linux/macOS keystores; consumer-side verbatim+invariant tests (envvault/protect/mkauth) are the deterministic guard for the Cli routing half. |
| **Two-arg `ArgumentException(msg, paramName)` throw-form class** (still deferred) | A SEPARATE class: the auto-appended `(Parameter 'x')` renders as SR key `Arg_ParamName_Name` when `.Message` is printed. **Grep-pattern lesson (SFH-1):** the leak also occurs with the 3-arg `ArgumentOutOfRangeException(paramName, value, message)` form — the `Arg_ParamName_Name, key` tail rides the SAME line as the message (verified by probe), so a caller's newline-trim does NOT strip it. Grep for BOTH the 2-arg `ArgumentException` and the 3-arg `ArgumentOutOfRangeException` forms when hunting this class. Reachable instance still deferred: `schedule/CrontabParser.cs:142,308` → printed at `Program.cs:193`; the clean fix conflicts with `ex.ParamName` security-pin tests (R3 crontab-injection guards). **Resolved instances of this class:** `mkauth/BasicAuthBuilder` (fixed `393733b`) and `digest/HmacFactory.cs:94` BLAKE2b oversized-key (fixed 2026-06-07, C2 — switched to single-arg `ArgumentException` + deleted the caller's now-unnecessary newline-trim). A large set of *latent* guard/DI-seam throws use these forms but don't reach user output; suite-wide remediation deferred as its own effort. |
