...
- **serve** (`hcat serve [dir]`, or bare `hcat`): static files + auto directory listing. `--upload` enables a POST receiver that saves to a dedicated `./uploads` directory (created but **not** served); `--upload-dir <path>` relocates it; `--upload-dir .` (the served root) is the deliberate escape hatch that makes uploads downloadable. Path traversal outside the served/upload roots is rejected. `--spa` enables single-page-app fallback: a request matching no file that is a browser navigation (`Accept: text/html`) returns the index file (`index.html`, or `--spa-index <file>` — a bare filename) with `200`, so client-side routers survive deep-link refresh. Missing assets/APIs (`*/*`, `application/json`) still `404`; existing files are unaffected; directory listing is disabled in `--spa` mode. Example: `hcat serve ./dist --spa`.
