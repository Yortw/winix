...
digest expands them itself on Windows — `digest *.dll` works the same as in bash.
...
so you get the normal "not found" error. In cmd, quoting a pattern (`"*.dll"`)
suppresses expansion; PowerShell removes quotes before digest sees them, so use `--%`
there if you need a literal. On Linux/macOS your shell expands wildcards as usual and
digest does nothing extra.
